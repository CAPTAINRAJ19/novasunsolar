...
const CopyableItem = ({ icon: Icon, label, value, href, theme, isEmail = false, isWebsite = false }) => {
  ...
  const handleClick = (e) => {
    if (isEmail) {
      e.preventDefault();
      window.open(`https://mail.google.com/mail/?view=cm&fs=1&to=${value}&su=Inquiry%20about%20Solar%20Solutions&body=Dear%20Novasun%20Solar%20Team,%0D%0A%0D%0AI%20am%20interested%20in%20your%20solar%20energy%20solutions.%20Please%20provide%20more%20information.%0D%0A%0D%0AThank%20you.`, '_blank');
    } else if (isWebsite) {
      e.preventDefault();
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex-1" onClick={handleClick}>
          <div className="flex items-center mb-3">
            <motion.div 
              ...
              className={`w-12 h-12 flex items-center justify-center rounded-xl mr-4 ${
                theme === 'dark' ? 'bg-yellow-400/20 text-yellow-400' : 'bg-orange-100 text-orange-600'
              }`}
            >
          ...
          <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'} leading-relaxed break-all`}>
            {value}
          </p>
        ...
        <motion.button
          ...
          className={`ml-4 p-3 rounded-xl transition-all duration-300 ${
            copied 
              ...
              : (theme === 'dark' ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')
          }`}
...
const Contact = () => {
  ...
  // Company features
  const features = [
    { icon: Award, title: '15+ Years Experience', description: 'Over a decade of infrastructure excellence and solar expertise' },
    { icon: Shield, title: 'Trusted Partnership', description: 'A unit of Soil and Brick Infraventures Pvt Ltd' },
    { icon: Users, title: '500+ Happy Customers', description: 'Serving communities across India with reliable solar solutions' },
    { icon: Clock, title: '24/7 Support', description: 'Round-the-clock customer service and technical assistance' }
  ];
      ...
      <div className="relative z-10">
              ...
              <motion.div variants={fadeInUp} className="space-y-8">
                ...
                >
                  <motion.div animate={pulse}>
                    <MessageCircle className={`w-8 h-8 mr-3 ${theme === 'dark' ? 'text-yellow-400' : 'text-orange-600'}`} />
                  </motion.div>
                  <span className={`${theme === 'dark' ? 'text-yellow-300' : 'text-orange-800'} font-semibold text-lg`}>Get in Touch</span>
                </motion.div>
                ...
                <motion.p 
                  ...
                  variants={fadeInUp}
                >
                  Ready to harness the power of the sun? Connect with our solar experts today. Every contact detail below is copyable for your convenience.
                </motion.p>
                ...
                >
                  <motion.div variants={fadeInUp} className={`px-6 py-3 rounded-full ${theme === 'dark' ? 'bg-yellow-400/20' : 'bg-orange-100'} backdrop-blur-sm`}>
                    <span className={`${theme === 'dark' ? 'text-yellow-300' : 'text-orange-700'} font-medium`}>✓ Click to Copy</span>
                  </motion.div>
                  <motion.div variants={fadeInUp} className={`px-6 py-3 rounded-full ${theme === 'dark' ? 'bg-yellow-400/20' : 'bg-orange-100'} backdrop-blur-sm`}>
                    <span className={`${theme === 'dark' ? 'text-yellow-300' : 'text-orange-700'} font-medium`}>✓ Instant Email</span>
                  </motion.div>
                  <motion.div variants={fadeInUp} className={`px-6 py-3 rounded-full ${theme === 'dark' ? 'bg-yellow-400/20' : 'bg-orange-100'} backdrop-blur-sm`}>
                    <span className={`${theme === 'dark' ? 'text-yellow-300' : 'text-orange-700'} font-medium`}>✓ Quick Access</span>
                  </motion.div>
                </motion.div>
              </motion.div>
            </motion.div>
        ...
        <section className={`py-16 ${theme === 'dark' ? 'bg-black/20 backdrop-blur-sm' : 'bg-gradient-to-r from-orange-100 via-yellow-50 to-orange-100'}`}>
          <div className="container mx-auto px-4">
            ...
            >
              <div className={`inline-flex items-center justify-center p-8 rounded-3xl shadow-2xl border backdrop-blur-md ${theme === 'dark' ? 'bg-gradient-to-br from-[#2f4ecc]/40 to-gray-800/40 border-white/20' : 'bg-white/80 border-yellow-200 shadow-yellow-100/50'}`}>
                <motion.div animate={floating} className="mr-6">
                  <Sun className={`w-16 h-16 ${theme === 'dark' ? 'text-yellow-400' : 'text-orange-600'}`} />
                </motion.div>
                <div className="text-left">
                  <h2 className={`text-3xl font-bold mb-2 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                    Novasun Solar
                  </h2>
                  <p className={`text-lg ${theme === 'dark' ? 'text-yellow-300' : 'text-orange-600'} font-semibold mb-1`}>
                    Let's contribute to the earth
                  </p>
                  <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                    A division of Soil and Brick Infraventures Pvt Ltd, specializing in solar energy solutions with over 15 years of infrastructure excellence.
                  </p>
                </div>
              </div>
            </motion.div>
          ...
          <div className="container mx-auto px-4">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="text-center mb-16">
              <h2 className="text-4xl lg:text-5xl font-bold mb-6">
                Contact <span className={theme === 'dark' ? 'text-yellow-400' : 'text-orange-600'}>Information</span>
              </h2>
              <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'} text-xl max-w-3xl mx-auto`}>
                Click any item to copy to clipboard. Email and website links will open directly.
              </p>
            </motion.div>
            ...
            >
              <motion.div variants={fadeInUp}>
                <CopyableItem 
                  ...
                  value="CIN: U70101GJ2015PTC083121"
                  theme={theme}
                />
              </motion.div>
              ...
              <motion.div variants={fadeInUp}>
                <CopyableItem 
                  ...
                  value="A unit of Soil and Brick Infraventures Pvt Ltd"
                  theme={theme}
                />
              </motion.div>
              ...
              <motion.div variants={fadeInUp} className="md:col-span-2">
                <CopyableItem 
                  ...
                  value="Plot no. 4, Ganapati Vihar, Khargapur, Gomti Nagar Ext., Lucknow-226010"
                  theme={theme}
                />
              </motion.div>
              ...
              <motion.div variants={fadeInUp}>
                <CopyableItem 
                  ...
                  value="jayantk735@gmail.com"
                  href="mailto:jayantk735@gmail.com"
                  theme={theme}
                  isEmail={true}
                />
              </motion.div>
              ...
              <motion.div variants={fadeInUp}>
                <CopyableItem 
                  ...
                  value="info@novasunsolar.com"
                  href="mailto:info@novasunsolar.com"
                  theme={theme}
                  isEmail={true}
                />
              </motion.div>
              ...
              <motion.div variants={fadeInUp} className="md:col-span-2">
                <CopyableItem 
                  icon={Globe}
                  label="Website"
                  value="www.novasunsolar.com"
                  href="https://www.novasunsolar.com"
                  theme={theme}
                  isWebsite={true}
                />
              </motion.div>
        ...
        <section className={`py-24 ${theme === 'dark' ? 'bg-black/20 backdrop-blur-sm' : 'bg-gradient-to-br from-yellow-50 via-orange-50 to-yellow-50'}`}>
          <div className="container mx-auto px-4">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="text-center mb-16">
              <h2 className="text-4xl lg:text-5xl font-bold mb-6">
                Why Choose <span className={theme === 'dark' ? 'text-yellow-400' : 'text-orange-600'}>Us?</span>
              </h2>
              <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'} text-xl max-w-3xl mx-auto`}>
            ...
            >
              {features.map((feature, index) => (
                ...
                >
                  ...
                  >
                    <feature.icon className={`w-10 h-10 ${theme === 'dark' ? 'text-gray-900' : 'text-white'}`} />
                  </motion.div>
          ...
          <div className="container mx-auto px-4 text-center relative z-10">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp}>
              <motion.div 
                ...
                className="mb-8"
              >
                <Heart className={`w-24 h-24 mx-auto ${theme === 'dark' ? 'text-yellow-400' : 'text-white'}`} />
              </motion.div>
              ...
              <p className={`text-xl lg:text-2xl mb-12 max-w-4xl mx-auto ${theme === 'dark' ? 'text-gray-200' : 'text-white/90'} leading-relaxed`}>
                Your journey to clean, renewable energy starts with a simple conversation. Reach out to us today and discover how solar can transform your energy future.
              </p>
              ...
              <motion.div 
                ...
                className={`text-lg ${theme === 'dark' ? 'text-gray-300' : 'text-white/80'}`}
              >
                ✓ Free Consultation • ✓ Custom Solutions • ✓ 15+ Years Experience • ✓ 24/7 Support
              </motion.div>
            </motion.div>
          </div>
